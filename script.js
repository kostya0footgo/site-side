...
window.addEventListener('load', function() {
  ...
  canvas.height = Math.max(900, processSection.offsetHeight); // минимум 900px
}
  resizeCanvas();
  ...
  // ВАЖНО: объявляем переменные ДО render()
  let progress = 0;
  let prevProgress = 0;

ScrollTrigger.create({
  trigger: document.getElementById('process'),
  start: 'top 800%',       // когда верх cardsGrid дойдёт до 80% экрана - начало
  end: 'bottom -100%',      // когда низ cardsGrid дойдёт до 20% экрана - конец
  scrub: 20,
  onUpdate: (self) => {
...
(function() {
  ...
  gsap.fromTo('.gsap-card', 
    ...
    {
      y: 0,
      rotation: (i) => [10, -20, 5][i] || 0,
      opacity: (i) => [0.7, 0.85, 1][i] || 1,
      scale: (i) => [0.95, 0.98, 1][i] || 1,
      stagger: 0.2,
      duration: 1.2,
      ease: 'power3.out',
      scrollTrigger: {
  ...
  cards.forEach((card, i) => {
    ...
    card.addEventListener('mouseleave', () => {
      const originalY = [0, 10, 20][i] || 0;
      const originalRotation = [-4, 2, -1][i] || 0;
      const originalScale = [0.95, 0.98, 1][i] || 1;
...
(function() {
  ...
  // Клонируем первую карточку в конец и последнюю в начало
  ...
  // Стартуем на первой настоящей карточке
  ...
  cont.addEventListener('scroll', function() {
    ...
    // Дошли до клона последней (в начале)
  ...
  // Восстановить scroll-snap после инициализации
  setTimeout(function() {
